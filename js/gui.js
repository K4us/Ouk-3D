...
(function () {
	...
	function initInfo() {
		...
		$info = $("<div>")
			...
			.appendTo($("body"))
			.css("left","auto")
			.css("right",window.innerWidth/2 + "px");
	}
	...
	function initGUI() {
		...
		makeButton("ABOUT",aboutDialog,$menu);
		makeButton("EXIT",exit,$menu);
	...
	function aboutDialog() {
		...
		var $aboutGame = $("<div>")
			.attr("id",id)
			.attr("title","ENG Raksa")
			.appendTo($("body"));

		$('<h2>Student at *Institute of Technology of Cambodia* (ITC).</h2>')
			.attr("id","t1")
			.appendTo($aboutGame);
		$('<h2>Programming in PHP, HTML5, JAVASCRIPT.</h2>')
			.attr("id","t2")
			.appendTo($aboutGame);
	...
	function exit() {
		//window.location.replace("http://infinite-dawn-3488.herokuapp.com/");
	}
	...
	function displayCheck() {
		if (validMoves.length === 0) {
			$info.text(( g_inCheck ? 'END' : 'OVER' ));
			//$info.text(( g_inCheck ? 'CHECKMATE' : 'STALEMATE' ));
		} else if (g_inCheck) {
			$info.text('KING IN DANGER');
			//$info.text('CHECK');
		} else {
			$info.text('');
		}
